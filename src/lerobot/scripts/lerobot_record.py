...
def record(cfg: RecordConfig) -> LeRobotDataset:
    ...
    if isinstance(robot, UR5ePGI) and isinstance(teleop, (SpnavTeleop, QuestTeleop)):
        if robot.config.action_mode != teleop.config.action_mode:
            raise ValueError(
                ...
                f"({robot.config.action_mode=} != {teleop.config.action_mode=}). "
                "Use matching values, for example `--robot.action_mode=eef --teleop.action_mode=eef`."
            )
    elif isinstance(robot, BiUR5ePGI) and isinstance(teleop, QuestTeleop):
        left_mode = robot.left_arm.config.action_mode
        right_mode = robot.right_arm.config.action_mode
        if left_mode != teleop.config.action_mode or right_mode != teleop.config.action_mode:
            raise ValueError(
                "BiUR5e PGI and Quest action modes must match when recording with teleop "
                f"({left_mode=}, {right_mode=}, {teleop.config.action_mode=})."
            )
    ...
    try:
        ...
        with VideoEncodingManager(dataset):
            initial_episode_count = dataset.num_episodes
