...
def make_teleoperator_from_config(config: TeleoperatorConfig) -> "Teleoperator":
    ...
    elif config.type == "spnav":
        from .spnav import SpnavTeleop

        return SpnavTeleop(config)
    elif config.type == "quest":
        from .quest import QuestTeleop

        return QuestTeleop(config)
    ...
    elif config.type == "keyboard_ur5e":
        from .keyboard.teleop_keyboard import KeyboardUR5eTeleop

        return KeyboardUR5eTeleop(config)
    ...
    elif config.type == "bi_gello_leader":
        from .bi_gello_leader import BiGelloLeader

        return BiGelloLeader(config)
